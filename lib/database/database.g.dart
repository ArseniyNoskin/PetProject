    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Person` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `username` TEXT NOT NULL, `email` TEXT NOT NULL, `password` TEXT NOT NULL, `age` INTEGER NOT NULL)');
        await database.execute(
...
class _$PersonDao extends PersonDao {
  ...
  Future<Person?> findPersonByEmail(String email) async {
    return _queryAdapter.query('SELECT * FROM Person WHERE email = ?1',
        mapper: (Map<String, Object?> row) => Person(
            row['id'] as int?,
            row['username'] as String,
            row['email'] as String,
  ...
  Future<void> updatePassword(
    ...
    await _queryAdapter.queryNoReturn(
        'UPDATE Person SET password =?2 WHERE email =?1',
        arguments: [userEmail, newPassword]);
  }
